...
| `order.user` | **Does not exist.** `order.user.id` and `order.user.custom.*` render empty even when the order has a user. Inside an Order or Orderline custom fields script the order's user is the **`user` global** (`user.id`, `user.custom.<field>`). Elsewhere, buyer identity is `order.email`, `order.first_name`, `order.last_name`, `order.telephone`. *Verified by query, 2026-09-22.* |
