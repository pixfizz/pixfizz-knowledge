...
nano_banana / Gemini image output carries an invisible **SynthID** provenance watermark. That is a fact, not something to confirm: assume every still generated through the Nano Banana family is watermarked. It is harmless on a preview plate, which never reaches production. Disclosure obligations for AI-generated advertising content vary by jurisdiction and are actively changing. Pixfizz does not provide legal advice on this. Confirm your own position before running AI imagery in paid advertising.

### A5. Keep AI imagery out of production files `[PIXFIZZ]` `[RULE]`

This file is about *marketing* assets only. Nothing generated by these tools should ever enter a **production output file**, a **design tool asset library that customers can place in their own projects**, or any **template set that contributes to fulfillment**. Licensing of AI-generated content for downstream commercial resale by a third party (your customer) is a separate and unresolved question.

A scene image inside a set marked `preview="true" fulfillment="false"` **is permitted**, because it renders only the on-screen preview and never reaches production. A1 still applies in full: the product, the frame, the paper, the binding and the finish are never generated, only the room around them.

*Verified by reading source (template definition semantics, `19_XML_TEMPLATE_REFERENCE.md`). Wording approved by the owner, 2026-09-03.*

| What A5 protects against | Preview-only scene |
|---|---|
| AI imagery reaching a production output file | Cannot. The set is `fulfillment="false"` |
| A customer placing AI assets into their own project | Cannot. It is a fixed page background, not an asset-library item |
| Unresolved resale licensing downstream | Not engaged. It is a backdrop behind the customer's own photo |

Worked example — a framed print 8x10 vertical. The generated room scene sits in the second set only, and production receives the plain print page:
...
measurement **and** inconsistent with each other between runs, it has reached its ceiling. Declare it
so and hand the question to a human eye. Iterating on a measurement that cannot agree with itself
produces confident numbers and no knowledge — and each further iteration makes the wrong number
