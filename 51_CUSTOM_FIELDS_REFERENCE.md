...
- **Large content: snippet-type only.** A snippet-type custom field holds 20,000+ characters, returned byte-clean via `item.custom.<field>` with no entity encoding and round-tripping through `parse_json` intact. A **text-type field does not: it caps at 256 characters** (stated by Alex, 2026-09-22; supersedes the earlier ~1KB estimate). **`multitext` is a list of short values with an add button, not a multi-line text box**; a single long string written to it does not store as expected (verified in admin, 2026-09-22). **A custom field's type cannot be changed after creation**: delete the definition and recreate it with the same name, which drops stored values, so say so first (stated by Alex). Snippet-type fields are conventionally used to *reference* a snippet but can also hold content directly. This is a separate limit from the ~2KB cap on **template options**.
